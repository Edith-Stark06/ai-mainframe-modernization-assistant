...
def test_generate_recommendations_high_complexity() -> None:
    ...
    score = ModernizationScore(complexity_score=0.9, coupling_score=0.1, overall_readiness=0.5)
...
def test_generate_recommendations_ready() -> None:
    flow = Flow(id="f1", name="F1", nodes=[], edges=[])
    score = ModernizationScore(complexity_score=0.1, coupling_score=0.1, overall_readiness=0.9)
...
def test_generate_recommendations_multiple_and_ordering() -> None:
    ...
    score = ModernizationScore(complexity_score=0.9, coupling_score=0.8, overall_readiness=0.2)
...
def test_recommendation_to_dict() -> None:
    rec = Recommendation(id="1", title="Title", description="Desc", priority=Priority.MEDIUM)
    d = rec.to_dict()
    assert d["id"] == "1"
