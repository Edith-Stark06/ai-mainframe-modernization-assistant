...
"""
    ...
    assert rules[0].condition == "A > B"
    assert rules[0].actions == ("MOVE 1 TO X",)
    # Ensure source location precisely matches the actual node generated by parser
    assert prog.procedure_division is not None
    para = prog.procedure_division.paragraphs[0]
    if_stmt = para.statements[0]
    move_stmt = if_stmt.then_statements[0]

    assert rules[0].source_location == move_stmt.start_position
