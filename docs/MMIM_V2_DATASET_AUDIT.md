...
`dataset_version = mmim-v2` · `generator_version = mmim-gen-v25` · `prompt_version = mmim-prompt-v2`
...
**Revision note — this is the twenty-fourth regeneration of `mmim-v2`.** `dataset_version` has stayed
...
**current (`mmim-gen-v25`)** values; §"Task-by-task quality assessment" below notes v1→v2 deltas
...
- **`mmim-gen-v25` (Stage 30, `docs/FIXED_FORMAT_NORMALIZATION.md`)**: 160 → 140 risk findings. 20 sources lose the
  `SYNTAX_ERROR` risk entirely — its only evidence was a false `SYN003` produced by a column-7 `*` comment line; 6 sources
  (`t_batch_acct_update`, `t_daily_trans_report`, `t_inventory_extract`, `t_inventory_reorder`, `t_payroll_file_post`,
  `t_table_indexed`) keep it for a genuine `SYN001`/`SYN005` and lose one occurrence each. Severity distribution now
  `{LOW: 13, MEDIUM: 12, HIGH: 20}` (verified directly against the regenerated `all.jsonl`; the pre-Stage-30 file read
  `{LOW: 12, MEDIUM: 13, HIGH: 20}`, so the older `{12, 12, 21}` figures above are historical).
...
- **`mmim-gen-v25` (Stage 30)**: distribution is now `REFACTOR 17 (37.8%)`, `REHOST 16 (35.6%)`,
  `PHASED_MIGRATION 6 (13.3%)`, `SERVICE_EXTRACTION 3 (6.7%)`, `REWRITE 2 (4.4%)`, `REPLATFORM 1 (2.2%)` (was `REHOST 19`,
  `REFACTOR 13`, `PHASED_MIGRATION 7`). The false `SYN003` from column-7 comment lines had pushed 5 sources onto a worse
  label: `t_account_eligibility`, `t_condition_names_88`, `t_customer_record`, `t_fallthrough_flow` move `REHOST` →
  `REFACTOR`, `t_payroll_deduct` moves `PHASED_MIGRATION` → `REHOST`; `t_credit_approval`, `t_pricing_tier` and
  `t_tax_withhold` keep `PHASED_MIGRATION` with changed evidence. The `t_condition_names_88` low-confidence fallback
  rationale described below was a *consequence of the false error*: with it removed the source now reads "parse
  complete, 0 unsupported constructs" at confidence 0.8. The label distribution was not rebalanced; it is reported as
  computed.
- **READY** — (as of `mmim-gen-v24`; at `mmim-gen-v25` REFACTOR 17 narrowly overtakes REHOST 16, see above) REHOST is
  the plurality label, still explainable by the sources that carry
...
| Strategy labels not pathologically dominated without explanation | **PASS** — REHOST 42% at `mmim-gen-v24` (down from 62% three cycles ago; 36% at `mmim-gen-v25`, where REFACTOR 38% is now the plurality) and explained (parser-limitation correlation, directly confirmed each cycle), 6 distinct labels present |
...
Stage 30 adds `tests/parser/test_normalizer_position_preserving.py` (new -- 25 tests) and
`tests/analysis/test_fixed_format_analysis.py` (new -- 62 tests; 59 fail on the isolated pre-fix tree, the other 3 pin behavior that must
not change); it regenerates the dataset (`mmim-gen-v25`) and the derived `instruction/` export, and updates 2 assertions in
`tests/dataset/test_mmim_v2_dataset.py` plus adds 4 Stage 30 pins there, and 3 assertions in `tests/modernization`/`tests/parser`/`tests/backend`, each a
direct, mechanically-traced consequence of the false `SYN003` disappearing -- never a blanket weakening (docs/FIXED_FORMAT_NORMALIZATION.md §6-§8).
